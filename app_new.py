...
class RAGChatbotApp:
    ...
    def __init__(self):
        ...
        try:
            self.chroma_setup = ChromaDBSetup(
    ...
    def upload_documents(
        ...
        if not self.chroma_setup:
            return "❌ ChromaDB not initialized. Please check logs.", self.get_stats()
        ...
        try:
            ...
            if all_chunks:
                ...
                upsert_stats = self.chroma_setup.upsert_chunks(all_chunks)
                
                # Update last upload time
                self.last_upload_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                
                # Reinitialize retriever to pick up new documents
                try:
                    self.retriever = Retriever(
                        chroma_db_path=CHROMA_DB_PATH,
                        collection_name=COLLECTION_NAME,
    ...
    def chat(
        ...
        if not self.retriever:
            response = "❌ Retriever not initialized. Please check if ChromaDB is set up correctly."
        ...
        try:
            ...
            retrieved_chunks = self.retriever.get_top_k(message, k=k_value)
...
"""
    ...
    def clear_conversation(self) -> Tuple[List, str, str]:
        ...
        Returns:
            Tuple of (empty_history, empty_chunks, updated_stats)
        """
...
def main():
    ...
    Instructions:
    1. Ensure ChromaDB is set up: python embeddings_and_chroma_setup.py
    2. Run this app: python app.py
    3. Open browser: http://localhost:7860
    4. Upload documents in the right panel
    ...
    try:
        ...
        print(f"\n❌ Error: {str(e)}")
        print("\nTroubleshooting:")
        print("1. Make sure ChromaDB is set up: python embeddings_and_chroma_setup.py")
        print("2. Check that all dependencies are installed: pip install -r requirements.txt")
        print("3. Verify port 7860 is not already in use")
